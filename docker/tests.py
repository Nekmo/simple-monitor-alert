...
def test_image(name, image_output=False, container_output=False):
    ...
    dir = os.path.join(__dir__, name)
    image_name = name.lower()
    execute(['docker', 'build', '-t', image_name, dir], image_output)
    execute(['docker', 'run', '--rm', image_name], container_output)
...
if __name__ == '__main__':
    ...
    if args.image:
        test_image(args.image, args.image_output, args.container_output)
    else:
